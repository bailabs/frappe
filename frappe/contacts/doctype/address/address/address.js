...
/*frappe.ui.form.on("Address", {
	validate_address:function(frm){ 		
		...
		$.ajax({
			url: 'http://172.105.158.123:8083/'+"https://onlinetools.ups.com/rest/XAV",
			type: "POST",
	...
	suggestions:function(frm){
		console.log("in suggestion line 103")
		...
		console.log("in suggestion in line 112") 
		console.log(array)        
	},
	refresh: function(frm) {		
		 //google API 
		 console.log("in suggestion line 119")
		 ...
		 $('input[data-fieldname="address_line1"]').autocomplete({
			 source: function( request, response ) {
				 ...
				 var desc=[]
				 $.ajax({
					 url: 'http://172.105.158.123:8083/'+"https://maps.googleapis.com/maps/api/place/autocomplete/json?input="+g_address+"&key=AIzaSyAFso5WNRELV-7xzSxV5FdHr1BCwqn4hD8",
					 type: 'POST',
					 dataType: 'json',
					 success: function (data, textStatus, xhr) {
						console.log("in suggestion line 131")
						 for (var i = 0; i < data.predictions.length; i++) {
							 desc.push(data.predictions[i].description)
						 } 
						 //console.log(desc)
						 response( $.grep( desc, function( item ){
							console.log("in suggestion line 137")
							 return item;
						 }) );
				 ...
				 //var g_address=$('input[data-fieldname="address_line1"]').val();
				console.log("in suggestion line 149")
				 ...
				 $.ajax({
					 url: 'http://172.105.158.123:8083/'+"https://maps.googleapis.com/maps/api/geocode/json?address="+g_address+"&key=AIzaSyAFso5WNRELV-7xzSxV5FdHr1BCwqn4hD8",
					 type: 'POST',
					 dataType: 'json',
					 success: function (data, textStatus, xhr) {
						//console.log("in google api line 168")
						//console.log(data)
						 ...
						 for (var i = 0; i < add_compo.length; i++) {
							...
							if (componentForm[addressType]) {  
								...
								cur_frm.set_value(componentForm1[addressType],val).then(function(){
									//console.log("in value google apia")  
									//console.log(val)
